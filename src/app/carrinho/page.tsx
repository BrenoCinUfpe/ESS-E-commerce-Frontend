...
export default function Carrinho() {
  ...
  // Caso não haja uma sessão válida, redireciona para a Home
                  ...
                  : cart && cart.products && cart.products.length > 0
                    ? cart.products.map(item => (
                        <div key={`${item.cartId}/${item.productId}`}>
                          <div className="w-full h-[145px] p-2 flex">
                            ...
                            <div className="ml-3 h-full w-full relative overflow-hidden">
                              ...
                              <div className="w-full h-[63px] flex items-end">
                                <p className="font-abeezee text-2xl italic flex-1">{`R$ ${floatToMoney(productData.find(product => product.id === item.productId)?.price ?? 98)}`}</p>
                                <div className="flex bg-projGray rounded-full w-40 pt-1 pb-1 justify-center items-center">
                                  ...
                                  <p className="font-abeezee text-md flex-1 flex justify-center items-center">{`${item.quantity}`}</p>
                                  <p className="cursor-pointer font-abeezee text-3xl flex-1 flex justify-center items-center rounded-full" onClick={() => { changeQuantity(item.productId, item.quantity, 1) }}>+</p>
                                </div>
                              </div>
                            </div>
                          ...
                          <div className="w-full h-[1px] bg-projGray mt-2 mb-2" />
                        </div>
                      ))
                    : <div className="w-full h-48 flex justify-center items-center opacity-75">
                        <TbShoppingCartQuestion size={35} />
                        <p className="pl-2 font-abeezee">Seu carrinho está vazio.</p>
                      </div>
                }
              </div>
            </div>
